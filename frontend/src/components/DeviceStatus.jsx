...
function SignalBars({ rssi }) {
  if (rssi == null) return <span className="text-slate-300 text-sm">—</span>
  const level = rssi >= -50 ? 4 : rssi >= -60 ? 3 : rssi >= -70 ? 2 : rssi >= -80 ? 1 : 0
  const color  = ['bg-red-400', 'bg-red-400', 'bg-orange-400', 'bg-yellow-400', 'bg-emerald-500'][level]
    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-end gap-1">
        {[1, 2, 3, 4].map(i => (
          <div
            key={i}
            className={`w-2.5 rounded-sm ${i <= level ? color : 'bg-slate-100'}`}
            style={{ height: `${8 + i * 5}px` }}
          />
        ))}
      </div>
      <p className="text-xs text-slate-500">{label} · {rssi} dBm</p>
    </div>
  )
}

function HeapBar({ freeHeap }) {
  if (freeHeap == null) return <span className="text-slate-300 text-sm">—</span>
  const totalKb = 320
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <p className="text-xs text-slate-500">{freeKb} KB libres · {pct}%</p>
    </div>
  )
}
...
export default function DeviceStatus({ data, latest, deviceInfo, timestamps }) {
  ...
  return (
    <main className="flex-1 overflow-y-auto p-5 space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-slate-700 text-sm">Hardware</h3>
            {deviceInfo?.last_seen && (
              <span className="ml-auto text-xs text-slate-300">últ. boot: {deviceInfo.last_seen}</span>
            )}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-semibold text-slate-700 text-sm">Red</h3>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-slate-700 text-sm">Señal WiFi</h3>
          </div>
          <p className="text-3xl font-bold text-slate-800 mb-3 leading-none">
            {latest.rssi != null ? Math.round(latest.rssi) : '—'}
            <span className="text-base font-normal text-slate-400 ml-1">dBm</span>
          </p>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-slate-700 text-sm">Memoria libre</h3>
          </div>
          <p className="text-3xl font-bold text-slate-800 mb-3 leading-none">
            {latest.free_heap != null ? Math.round(latest.free_heap / 1024) : '—'}
            <span className="text-base font-normal text-slate-400 ml-1">KB</span>
          </p>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-slate-700 text-sm">Uptime</h3>
          </div>
          <p className="text-3xl font-bold text-slate-800 mb-1 leading-none">
      ...
      {/* Gráficas tiempo real (últimas 30 muestras) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <WeatherChart
